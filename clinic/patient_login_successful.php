...
if( isset($_POST['email']) && isset($_POST['password']) ){ //This checks if the email or password was sent over from the log in page to this profile page
	...
	if ($result = mysqli_query($conn,$query)){ //This checks if there is even a connection to the database
		...
		$query1 = "SELECT patient_id,full_name,password,email,dob,phone,gender FROM patient_registration WHERE email = '$email' AND password = '$password'";
...
?>			
			...
			<a id = "" href="Patient_Profile.php">Go To Profile</a>
		...
		?>
			<a id = "forgot_info_link" href="Login_Patient.php">Incrorrect email or password. Click here to go back</a>
		<?php	
			return 0;	
		}
